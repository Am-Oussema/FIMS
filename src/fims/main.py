...
def pretty_print_events(events):
    ...
    for e in events:
        counts[e["type"]]+=1
    print(f"CREATED: {counts['created']}, MODIFIED: {counts['modified']}, DELETED: {counts['deleted']}\n")
...
def cmd_compare(args):
    ...
    if snapshot is None:
        print(f"[-] No snapshot found for this folder. Create one first with: \n     python main.py create --path \"{folder}\"")
        return
    snap_folder=snapshot.get("folder")
    if os.path.abspath(snap_folder) != os.path.abspath(folder):
        print("[-] Snapshot does not match folder. Create a new snapshot for this folder.")
        return
...
def cmd_list(args):
    ...
    for s in snaps: 
        print(f"- {s.get("file")} (folder={s.get("folder")}, created_at={s.get("created_at")})")
...
def main():
    ...
    p_compare.add_argument("--json",action="store_true",help="Also print JSON summary")
